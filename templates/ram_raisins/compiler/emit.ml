...
and g' oc = function
	...
	| Tail, (Nop | St _ | StF _ | Comment _ | Save _ as exp) ->
		...
		Output.add_stmt (Output.B reg_lk)
	...
	| Tail, IfEq(x, C(0), e1, e2) ->
		g'_tail_if oc x reg_0 e1 e2 "beq" "bne"

	| Tail, IfEq(x, C(1), e1, e2) ->
		g'_tail_if oc x reg_p1 e1 e2 "beq" "bne"

	| Tail, IfEq(x, C(-1), e1, e2) ->
		g'_tail_if oc x reg_m1 e1 e2 "beq" "bne"
	...
	| Tail, IfFLE(x, y, e1, e2) ->
		g'_tail_if oc y x e1 e2 "fjge" "fblt"

	| NonTail(z), IfEq(x, V(y), e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) x (pp_id_or_imm (V y)) e1 e2 "beq" "bne"

	| NonTail(z), IfEq(x, C(0), e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) x reg_0 e1 e2 "beq" "bne"

	| NonTail(z), IfEq(x, C(1), e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) x reg_p1 e1 e2 "beq" "bne"

	| NonTail(z), IfEq(x, C(-1), e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) x reg_m1 e1 e2 "beq" "bne"
	...
	| NonTail(z), IfFLE(x, y, e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) y x e1 e2 "fjge" "fblt"
  ...
  | NonTail(a), CallCls(x, ys, zs) -> (* レジスタで飛ぶジャンプ *)
		...
		Output.add_stmt (Output.Subi (reg_sp, reg_sp, ss + 1));
		Output.add_stmt (Output.Ldi (reg_sw, reg_cl, 0));
		Output.add_stmt (Output.Jarl reg_sw);
		Output.add_stmt (Output.Addi (reg_sp, reg_sp, ss + 1)); 
