...
let output_stmt oc stmt =
	if stmt.state = Vanish then ()
		...
		match stmt.inst with
			...
			| FMvhi (dst, n) -> Printf.fprintf oc "\tflui\t%s, %d\n" dst n
			| FMvlo (dst, n) -> Printf.fprintf oc "\tflli\t%s, %d\n" dst n
			| Mvhi (dst, n) -> Printf.fprintf oc "\tlui\t%s, %d\n" dst n
			| Mvlo (dst, n) -> Printf.fprintf oc "\tlli\t%s, %d\n" dst n
			...
			| Ld (dst, src, index) -> Printf.fprintf oc "\tloadr\t%s, %s, %s\n" dst src index
			| Ldi (dst, src, index) -> Printf.fprintf oc "\tload\t%s, %s, %d\n" dst src (-index);
			| LdF (dst, src, index) -> Printf.fprintf oc "\tfload\t%s, %s, %s\n" dst src index
			| LdFi (dst, src, index) -> Printf.fprintf oc "\tfldi\t%s, %s, %d\n" dst src (-index);
			| St (src, target, index) -> Printf.fprintf oc "\tstorer\t%s, %s, %s\n" src target index
			| Sti (src, target, index) -> Printf.fprintf oc "\tstore\t%s, %s, %d\n" src target (-index);
			| StF (src, target, index) -> Printf.fprintf oc "\tfstore\t%s, %s, %s\n" src target index
			| StFi (src, target, index) -> Printf.fprintf oc "\tfsti\t%s, %s, %d\n" src target (-index);
			| Input src -> 	Printf.fprintf oc "\tiold\t%s\n" src
			| InputW src -> Printf.fprintf oc "\tinputw\t%s\n" src
			| InputF src -> Printf.fprintf oc "\tinputf\t%s\n" src
			| Output dst -> Printf.fprintf oc "\tiost\t%s\n" dst
